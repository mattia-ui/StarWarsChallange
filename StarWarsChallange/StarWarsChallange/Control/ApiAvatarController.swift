...
final class APIAvatarClient {
    //private let baseUrlString = "https://mobile.aws.skylabs.it/mobileassignments/swapi/"
    private let baseUrlString = "https://mobile.aws.skylabs.it/mobileassignments/swapi/placeholder.png"
    ...
    func fetch<T: Decodable>(with url: URL?, page: Int?, dataType: T.Type, completion: @escaping (Result<T, NetworkError>) -> Void) {
        ...
        if let url = url { request = url }
            ...
            //urlString = page == 1 ? baseUrlString : baseUrlString + "?page=\(page).png"
